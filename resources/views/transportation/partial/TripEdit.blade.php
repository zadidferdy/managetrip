...
@section('breadcrumb', 'Transportation / Trip / Edit')
...
<div class="page-header">
  <div>
    ...
    <div style="font-size:13px;color:var(--text-muted);margin-top:2px;">
      <span style="font-family:'Space Mono',monospace;font-weight:700;color:var(--primary);">{{ $trip->trip_code }}</span>
    </div>
  </div>
  ...
  <div class="card-body">
    <div class="grid-2">
      <div class="form-group">
        ...
        <input type="datetime-local" name="departure_datetime" class="form-control"
          value="{{ old('departure_datetime', \Carbon\Carbon::parse($trip->departure_datetime)->format('Y-m-d\TH:i')) }}" required>
      ...
      <div class="form-group">
        ...
        <input type="datetime-local" name="arrival_datetime" class="form-control"
          value="{{ old('arrival_datetime', $trip->arrival_datetime ? \Carbon\Carbon::parse($trip->arrival_datetime)->format('Y-m-d\TH:i') : '') }}">
    ...
    <div class="grid-2">
      <div class="form-group">
        ...
        <input type="text" name="from_location" class="form-control" value="{{ old('from_location', $trip->from_location) }}" required>
      ...
      <div class="form-group">
        ...
        <input type="text" name="to_location" class="form-control" value="{{ old('to_location', $trip->to_location) }}" required>
      ...
      <div class="form-group">
        ...
        <select name="trip_type" class="form-control" required>
          ...
          @foreach($tripTypes as $type)
            <option value="{{ $type }}" {{ old('trip_type', $trip->trip_type) == $type ? 'selected' : '' }}>{{ $type }}</option>
          @endforeach
        </select>
      ...
      <div class="form-group">
        ...
        <select name="priority" class="form-control" required>
          @foreach(['Flexible','Normal','Urgent','Emergency'] as $p)
            <option value="{{ $p }}" {{ old('priority', $trip->priority) == $p ? 'selected' : '' }}>{{ $p }}</option>
          @endforeach
        </select>
      </div>
      <div class="form-group">
        ...
        <select name="region" class="form-control">
          ...
          @foreach(['Kantor Pusat','Kantor Cabang','Tambang 1','Tambang 2','Tambang 3','Tambang 4','Tambang 5','Tambang 6'] as $r)
            <option value="{{ $r }}" {{ old('region', $trip->region) == $r ? 'selected' : '' }}>{{ $r }}</option>
          @endforeach
        </select>
      </div>
    </div>
    ...
    <div class="grid-2">
        ...
        @if(in_array(auth()->user()->role_user, ['manager', 'admin', 'admin_trans']))
          <select name="vehicle_id" class="form-control">
            ...
            @forelse($vehicles as $v)
              <option value="{{ $v->id }}"
                {{ old('vehicle_id', $trip->vehicle_id) == $v->id ? 'selected' : '' }}>
          ...
          @enderror
        @else
          {{-- Role lain: tampilkan info saja --}}
          @if($trip->vehicleRel)
            <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:10px 14px;font-size:13px;">
              <i class="ti ti-car" style="color:var(--primary);margin-right:6px;"></i>
              <strong>{{ $trip->vehicleRel->name }}</strong> — {{ $trip->vehicleRel->plate_number }}
              <span style="color:var(--text-muted);font-size:12px;margin-left:6px;">({{ $trip->vehicleRel->type }})</span>
            </div>
          @else
      ...
      <div class="form-group">
        ...
        @if(in_array(auth()->user()->role_user, ['manager', 'admin', 'admin_trans']))
          <select name="driver" class="form-control">
            ...
            @forelse($drivers as $d)
              <option value="{{ $d->nama_user }}"
                {{ old('driver', $trip->driver) == $d->nama_user ? 'selected' : '' }}>
                {{ $d->nama_user }}
              </option>
            @empty
        ...
        @else
          ...
          @if($trip->driver)
            <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:10px 14px;font-size:13px;">
              <i class="ti ti-user" style="color:var(--primary);margin-right:6px;"></i>
              {{ $trip->driver }}
            </div>
          @else
            <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:10px 14px;font-size:13px;color:var(--text-muted);">
      ...
      <div class="form-group" style="margin-bottom:0;">
        ...
        <div style="position:relative;">
          <input type="number" name="km_start" class="form-control" style="padding-right:48px;"
            ...
            min="0"
            value="{{ old('km_start', $trip->km_start) }}">
      ...
      <div class="form-group">
        ...
        <select name="approver1" class="form-control" required>
          ...
          @forelse($approvers1 as $user)
            <option value="{{ $user->nama_user }}" {{ old('approver1', $trip->approver1) == $user->nama_user ? 'selected' : '' }}>
      ...
      <div class="form-group">
        ...
        <select name="approver2" class="form-control" required>
          ...
          @forelse($approvers2 as $user)
            <option value="{{ $user->nama_user }}" {{ old('approver2', $trip->approver2) == $user->nama_user ? 'selected' : '' }}>
...
<div class="card" style="margin-bottom:20px;">
  ...
  <div class="card-body">
    <div class="form-group">
      ...
      <textarea name="purpose" class="form-control" rows="3" required>{{ old('purpose', $trip->purpose) }}</textarea>
      @error('purpose')<div style="color:var(--danger);font-size:12px;margin-top:4px;">{{ $message }}</div>@enderror
    </div>
    <div class="form-group" style="margin-bottom:0;">
      <label class="form-label">Catatan Tambahan</label>
      <textarea name="notes" class="form-control" rows="2">{{ old('notes', $trip->notes) }}</textarea>
    </div>
  </div>
</div>
